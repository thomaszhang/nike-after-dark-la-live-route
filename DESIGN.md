...
The course uses a 72%-opacity, 5 px Nike-red line with a restrained white edge. Repeated 14 px white CSS chevrons are anchored directly to sampled route coordinates, rotated to each segment bearing, and regenerated by zoom at approximately 30 screen-pixel intervals. Screen-space filtering preserves 24 px between chevrons and around the live location. Mile, START, and FINISH markers render above chevrons, masking the sequence cleanly where required. Where opposing passes share the same pixels, the selected travel direction alternates along the corridor so both directions remain visible without drawing crossed shapes.
